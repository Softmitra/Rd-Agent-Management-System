    ...
    <div id="importResults" class="card d-none">
        ...
        <div class="card-body">
            ...
            <div class="mt-4 text-center">
                <a href="{{ route('agent.rd-agent-accounts.index') }}" class="btn btn-primary">
                    <i class="fas fa-list"></i> View RD Accounts
                </a>
                <a href="{{ route('agent.customers.index') }}" class="btn btn-info">
                    <i class="fas fa-users"></i> View Customers
                </a>
                <button type="button" class="btn btn-warning" onclick="checkIncompleteRecords()">
                    <i class="fas fa-exclamation-triangle"></i> Complete Records
                </button>
...
async function loadAgents() {
    try {
        const response = await axios.get('{{ route("agent.excel-import.agents") }}');
...
// Handle form submission with password verification
document.getElementById('uploadForm').addEventListener('submit', async function(e) {
    ...
    const password = prompt('Enter your login password to proceed with Excel import:');
    if (!password) {
        showAlert('Password is required to proceed with import.', 'warning');
        return;
    }
    ...
    try {
        const response = await axios.post('{{ route("agent.excel-import.upload") }}', formData, {
            headers: {
...
async function loadExistingRdAccounts(customerId) {
    try {
        // Fetch RD accounts for this customer using agent route
        const response = await axios.get(`{{ url('agent/customers') }}/${customerId}/rd-accounts`);
        ...
        // Update the table cell - only update the RD accounts part, preserve status and remark
        const cell = document.querySelector(`[data-customer-id="${customerId}"]`);
        if (cell) {
            // Find existing status div and preserve it
            const statusDiv = cell.querySelector('.mt-2');
...
async function startImport() {
    ...
    // Collect agent assignments for duplicates
    const agentAssignments = {};
    const duplicateSelects = document.querySelectorAll('select[name^="agent_assignment_"]');
    ...
    duplicateSelects.forEach(select => {
        ...
        if (!agentId) {
            ...
            agentAssignments[index] = agentId;
        }
    });
    
    // Check if skipDuplicates checkbox exists before accessing it
    const skipDuplicatesCheckbox = document.getElementById('skipDuplicates');
    if (!allAssigned && (!skipDuplicatesCheckbox || !skipDuplicatesCheckbox.checked)) {
        showAlert('Please assign agents to all duplicate customers or check "Skip duplicates".', 'warning');
        return;
    }
    ...
    try {
        const response = await axios.post('{{ route("agent.excel-import.import") }}', {
            file_path: previewData.file_path,
            agent_assignments: agentAssignments,
            skip_duplicates: skipDuplicatesCheckbox ? skipDuplicatesCheckbox.checked : true,
...
function showImportResults(results) {
    ...
    // Auto-trigger incomplete records check after import
...
function showAlert(message, type) {
    ...
    const title = container.querySelector('h3').parentElement;
    if (title) {
...
// Check for incomplete records after import
async function checkIncompleteRecords() {
    try {
        ...
        if (data.count > 0) {
            showAlert(`Import completed! You have ${data.count} customers with incomplete details. Please complete them for proper record keeping.`, 'warning');
            
            // Redirect to customers page after 3 seconds
            setTimeout(() => {
                window.location.href = '{{ route("agent.customers.index") }}';
            }, 3000);
        } else {
            showAlert('All customer records are complete!', 'success');
        }
    } catch (error) {
